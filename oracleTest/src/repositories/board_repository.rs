...
impl BoardRepository {
    ...
    pub async fn insert(&self, title: String, content: String) -> Result<i64, oracle::Error> {
        ...
        spawn_blocking(move || {
            ...
            rows.next()
                .transpose()?
                .map_or_else(
                    || Err(oracle::Error::InternalError("Failed to get CURRVAL after insert".to_string())),
    ...
    /// 게시글 수정
    pub async fn update(&self, id: i64, title: String, content: String) -> Result<bool, oracle::Error> {
        ...
        spawn_blocking(move || {
            ...
            let params: [&dyn ToSql; 3] = [&title, &content, &id];
            let rows_affected = conn.execute(UPDATE_BOARD, &params)?.row_count()?;
            conn.commit()?;
    ...
    /// spawn_blocking 내부에서 사용하기 위해 &self 의존성을 제거함
